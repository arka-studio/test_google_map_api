...
const ERROR_EXPLANATIONS = {
  InvalidKeyMapError: {
    ...
    message: "API key salah, sudah dihapus, atau bukan key dari project Google Cloud yang benar.",
  },
  ApiNotActivatedMapError: {
    title: "Gagal: Maps JavaScript API belum aktif",
    message: "API key ada, tetapi Maps JavaScript API belum diaktifkan pada project Google Cloud tersebut.",
  },
  BillingNotEnabledMapError: {
    title: "Gagal: Billing belum aktif",
    message: "Google Maps Platform membutuhkan billing aktif. Aktifkan billing pada project Google Cloud yang memakai API key ini.",
  },
  RefererNotAllowedMapError: {
    title: "Gagal: Domain tidak diizinkan",
    message: "API key dibatasi oleh HTTP referrer, tetapi domain website ini belum masuk daftar izin. Tambahkan domain ini ke restriction API key.",
  },
  RequestDeniedMapError: {
    title: "Gagal: Request ditolak",
    message: "Google menolak request. Biasanya karena konfigurasi API key, billing, restriction, atau API service belum sesuai.",
  },
  GoogleMapsAuthFailure: {
    title: "Gagal: autentikasi Google Maps ditolak",
    message: "Google Maps memanggil auth failure. Artinya key tidak valid untuk domain ini, billing belum aktif, API belum aktif, atau restriction belum cocok.",
  },
  PlacesLibraryMissing: {
    title: "Gagal: Places library tidak tersedia",
    message: "Maps JavaScript API berhasil dimuat, tetapi library Places tidak tersedia. Aktifkan Places API dan pastikan script memuat libraries=places.",
  },
  GoogleMapsObjectMissing: {
    title: "Gagal: Google Maps object tidak tersedia",
    message: "Script Google Maps selesai dipanggil, tetapi object google.maps tidak tersedia. Biasanya karena key/API/restriction bermasalah.",
  },
  GoogleMapsScriptLoadError: {
    title: "Gagal: script Google Maps tidak bisa dimuat",
    message: "Browser gagal memuat script Google Maps. Cek koneksi, domain restriction, API key, atau kemungkinan script diblokir.",
  },
  GoogleMapsRuntimeError: {
    title: "Gagal: error saat membuat map",
    message: "Script berhasil dipanggil, tetapi gagal saat membuat map/autocomplete. Cek konfigurasi Maps JavaScript API dan Places API.",
  },
  Timeout: {
    title: "Gagal: waktu tes habis",
    message: "Google Maps tidak memberi respons valid dalam batas waktu. Cek koneksi, billing, API key, dan restriction domain.",
  },
  MissingKeyMapError: {
...
function setResult(type, title, message) {
  ...
  resultBox.innerHTML = `<div class="result-title">${escapeHtml(title)}</div><p>${escapeHtml(message)}</p>`;
}
...
function explainError(errorCode, fallbackMessage) {
  ...
  if (ERROR_EXPLANATIONS[detectedCode]) return ERROR_EXPLANATIONS[detectedCode];
  return {
    title: `Gagal: ${detectedCode || "Error tidak diketahui"}`,
    message: fallbackMessage || "Google Maps API gagal dites. Cek API key, billing, API yang aktif, dan restriction domain.",
  };
}
...
(function () {
  ...
  function post(ok, code, message) {
    ...
    parent.postMessage({ type: "GOOGLE_MAPS_API_TEST_RESULT", testId: testId, ok: ok, code: code, message: message || lastConsoleError || "" }, "*");
  }
  ...
  console.error = function () {
    ...
    if (code) post(false, code, lastConsoleError);
  ...
  window.addEventListener("error", function (event) {
    ...
    if (code) post(false, code, message);
  }, true);
  ...
  window[callbackName] = function () {
    try {
      ...
      var map = new google.maps.Map(mapElement, { center: { lat: -8.4095, lng: 115.1889 }, zoom: 10 });
      var input = document.getElementById("places-input");
      new google.maps.places.Autocomplete(input, { componentRestrictions: { country: "id" } });
  ...
  script.async = true;
  script.defer = true;
  script.onerror = function () { post(false, "GoogleMapsScriptLoadError", "Google Maps script failed to load."); };
  ...
  setTimeout(function () { post(false, "Timeout", lastConsoleError || "Google Maps test timeout."); }, 15000);
})();
<\/script>
</body></html>`;
}
...
function testGoogleMapsApiKey(apiKey) {
  ...
  return new Promise((resolve, reject) => {
    messageHandler = function (event) {
      ...
      if (data.type !== "GOOGLE_MAPS_API_TEST_RESULT" || data.testId !== currentTestId) return;
      if (data.ok) resolve(data);
      else reject(Object.assign(new Error(data.code || "GoogleMapsTestFailed"), data));
    };
    ...
    timeoutId = setTimeout(() => reject(new Error("Timeout")), 17000);
  }).finally(() => cleanupPreviousTest());
}
...
form.addEventListener("submit", async (event) => {
  ...
  setResult("loading", "Sedang mengetes lebih ketat...", "Tester membuat iframe terpisah, memuat Maps JavaScript API + Places, membuat map kecil, lalu menunggu auth failure dari Google.");

  try {
    await testGoogleMapsApiKey(apiKey);
    setResult("success", "Lolos: API key benar-benar berhasil", "Maps JavaScript API dan Places library berhasil dimuat, map berhasil dibuat, dan tidak ada auth failure yang terdeteksi untuk domain ini.");
  } catch (error) {
